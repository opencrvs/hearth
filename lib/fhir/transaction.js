  ...
  return {
    ...
    revertUpdate: (resourceType, id, callback) => {
      mongo.getDB((err, db) => {
        ...
        cHistory.findOneAndDelete({ id }, { sort: { '_transforms.meta.lastUpdated': -1 } }, (err, result) => {
          if (err) {
            return callback(err)
          }
          ...
          if (result.value) {
            ...
            c.findOneAndReplace({ id }, result.value, (err, result) => {
              if (err) {
                return callback(err)
              }
    ...
    revertDelete: (resourceType, id, callback) => {
      mongo.getDB((err, db) => {
        ...
        // remove deleted history entry
        cHistory.findOneAndDelete({ id, _request: { method: 'DELETE' } }, { sort: { '_transforms.meta.lastUpdated': -1 } }, (err) => {
          if (err) {
            return callback(err)
          }

          // move last non-delete entry to non-history collection
          cHistory.findOneAndDelete({ id }, { sort: { '_transforms.meta.lastUpdated': -1 } }, (err, result) => {
            if (err) {
              return callback(err)
            }
            // ensure findOneAndDelete returns deleted document
            if (!result.value) {
              return callback(null, false)
            }

            delete result.value._id
            c.findOneAndReplace({ id }, result.value, { upsert: true }, (err, result) => {
              if (err) {
                return callback(err)
              }
              callback(null, true)
