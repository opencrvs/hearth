...
function buildOperationOutcome (severity, code, detailsText, diagnostics) {
  return {
    ...
    issue: [
      {
        ...
        diagnostics: diagnostics || detailsText
      }
    ]
  }
}
...
module.exports = exports = (mongo) => {
  ...
  function mapSearchNameToPath (searchParameters) {
    ...
    _.castArray(searchParameters).forEach((parameter) => {
      promises.push(new Promise((resolve, reject) => {
        ...
        const itemResource = itemList[0]
        const itemReference = itemList[1]
  ...
  function performReverseInclude (revinclude, results) {
    ...
    if (!Object.keys(structDefMap).includes(resourceType)) {
    ...
    bundleResults: (type, entries, total, callingUrl) => {
      ...
      return {
        ...
        entry: entries.map((entry) => {
          ...
          if (entry._request && !entry._request.url) {
          ...
          let fullUrl = `${config.getConf('server:publicFhirBase')}/${entry.resourceType}/${entry.id}`

          if (entry.meta && entry.meta.versionId) {
            fullUrl = `${fullUrl}/_history/${entry.meta.versionId}`
          }
