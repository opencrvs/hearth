...
module.exports = (mongo, modules) => {
  ...
  return {
     ...
     * @param {RequestContext} ctx
     * @param {String} resourceType The type of FHIR resource
     * @param {String} id The FHIR resource id or null if this is a type level history operation
     * @param {CoreCallback} callback
     */
    history: (ctx, resourceType, id, callback) => {
      authorization.authorize('history', ctx, resourceType, (err, badRequest) => {
        ...
        hooks.executeBeforeHooks('history', fhirModule, ctx, resourceType, (err, badRequest) => {
          ...
          mongo.getDB((err, db) => {
            ...
            if (id) {
              query[1].$lookup.pipeline.push({ $match: { id } })
              query[2].$lookup.pipeline.push({ $match: { id } })
            }

            if (ctx.query._since) {
              const matchClause = { $match: { '_transforms.meta.lastUpdated': { $gt: new Date(ctx.query._since) } } }
        ...
        hooks.executeBeforeHooks('update', fhirModule, ctx, resourceType, resource, (err, badRequest) => {
          ...
          mongo.getDB((err, db) => {
            ...
            c.findOne({ id }, {}, (err, result) => {
              ...
              if (ctx.headers['if-match'] && ctx.headers['if-match'] !== `W/"${result.meta.versionId}"`) {
                return callback(null, fhirCommon.buildHTTPOutcome(409, 'error', 'conflict', 'Conflict: If-Match header must match resource versionId'))
              }
            ...
            c.findOneAndDelete({ id }, (err, result) => {
              ...
              if (ctx.query._purge !== 'true') {
                cHistory.insert(historyDoc, (err) => {
                  ...
                  // Also create a deleted record in the history per FHIR spec
                  const deleteDoc = {
                    id: historyDoc.id,
                  ...
                  fhirCommon.touchResource(deleteDoc)
